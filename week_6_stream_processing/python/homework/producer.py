...
from fhv_ride_record import FhvRideRecord, ride_record_to_dict
from fhv_ride_record_key import FhvRideRecordKey, ride_record_key_to_dict
...
class RideAvroProducer:
    ...
    def read_records(resource_path: str):
        ...
        with open(resource_path, 'r') as f:
            ...
            for row in reader:
                # ride_records.append(FhvRideRecord(arr=[row[0], row[3], row[4], row[9], row[16]]))
                ride_records.append(FhvRideRecord(arr=[row[0], row[1], row[2], row[3], row[4], row[5], row[6]]))
                ride_keys.append(FhvRideRecordKey(affiliated_base_number=str(row[6])))
        return zip(ride_keys, ride_records)

    def publish(self, topic: str, records: [FhvRideRecordKey, FhvRideRecord]):
